...
Make S+P chart - treatment vs control

```{r}
# tidy_norm <- norm_skud_evi %>%
#   filter(Experiment == "pellet") %>%
#   rename(pellet_norm = norm_value) %>%
#   select(-Experiment, -value) %>%
#   merge(., filter(norm_skud_evi, Experiment == "supernatant"), by = "ratio") %>%
#   rename(sup_norm = "norm_value") %>%
#   select(-Experiment, -value)

#ggplot(filter(norm_skud_evi, ratio = "")
```

Put them in a list? Not paired values...
...
```{r}
# s_and_p <- as.data.frame(gene_evi) %>%
#   pivot_wider(id_cols = c(gene), names_from = Experiment, 
#               values_from = c("Intensity L" , "Intensity M", "Intensity H")) %>%
#   unchop(cols = c("Intensity L_pellet", "Intensity L_supernatant", "Intensity M_pellet", 
#                   "Intensity M_supernatant", "Intensity H_pellet", "Intensity H_supernatant")) #%>%
#   mutate(spL = sum(as.numeric(`Intensity L_pellet`) + as.numeric(`Intensity L_supernatant`)),
#          spM = sum(as.numeric(`Intensity M_pellet`) + as.numeric(`Intensity M_supernatant`)),
#          spH = sum(as.numeric(`Intensity H_pellet`) + as.numeric(`Intensity H_supernatant`))) 
# 
# s_and_p <- as.data.frame(gene_evi) %>%
